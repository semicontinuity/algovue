test = function() {
    ...
    const shortestSubarray = vm.functionDeclaration(
        ...
        vm.sequenceStatement([
            vm.assignment(
                vm.varWrite('N'),
                vm.functionCall('length', [], 'A')
            ),
            vm.assignment(
                vm.varWrite('P'),
                vm.newIntArray(vm.expression(vm.plus(), vm.variable('N'), vm.number(1)))
            ),
            vm.assignment(
                vm.varWrite('i'),
                vm.number(0)
            ...
            vm.whileStatement(
                ...
                vm.sequenceStatement([
                    vm.assignment(
                        vm.arrItemWrite('P', vm.expression(vm.plus(), vm.variable('i'), vm.number(1))),
                        vm.expression(vm.plus(), vm.arrItem('P', vm.variable('i')), vm.arrItem('A', vm.variable('i')))
                    ),
                    vm.assignment(
                        undefined,
                        vm.varPostOp('i', true)
            ...
            vm.assignment(
                vm.varWrite('ans'),
                vm.expression(vm.plus(), vm.variable('N'), vm.number(1))
            ),
            vm.assignment(
                vm.varWrite('monoq'),
                vm.arrayLiteral([])
                ...
                vm.sequenceStatement([
                    ...
                    vm.whileStatement(
                        ...
                        vm.sequenceStatement([
                            vm.assignment(
                                vm.varWrite('ans'),
                                vm.functionCall('min', [vm.variable('ans'), vm.expression(vm.minus(), vm.variable('y'), vm.functionCall('shift', [], 'monoq'))], 'Math')
                            )
                        ])
                    ),
                    vm.assignment(
                        undefined,
                        vm.functionCall('push', [vm.variable('y')], 'monoq')
                    ),
                    vm.assignment(
                        undefined,
                        vm.varPostOp('y', true)
    ...
    const usage = vm.sequenceStatement([
        vm.assignment(
            vm.varWrite('result'),
            vm.functionCall(shortestSubarray, [vm.arrayLiteral([vm.number(2), vm.number(-1), vm.number(2)]), vm.number(3)])
        ),
        vm.stop()
    ]);
